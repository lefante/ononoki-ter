...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5511940642567@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'tts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 1000
					? reply('Texto muito grande, n vo ler isso não')
				...
				/**case 'meme':
					...
					buffer = await getBuffer(`https://imgur.com/${meme.loli}.jpg`)
				...
				/*case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'não faço a mínima ideia da tradução do que era pra estar aqui'})
					})
					break
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERROR* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'pão'})
					})
					break*/
				case 'hilih':
                 ...
                 case 'ping':
                   ...
                   client.sendMessage(from, `Pong! Speed: ${latensi.toFixed(4)} _Second_`, msgType.text, {quoted: mek})
                   break
                  case 'calma':
                  client.sendMessage(from, `Tabom`, msgType.text, {quoted: mek})
                  break
                case 'cu':
                  client.sendMessage(from, `Kkkkkkkk ele disse cukkkkkkkkkkkk`, msgType.text, {quoted: mek})
                  break
				case 'yt2mp3':
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Rebaixado igual um corsa, com sucesso\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Vai doer mais em mim do que em você :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedidos recebidos : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'image':
					if (!isQuotedSticker) return reply('❌ Responda a um sticker ❌')
					reply(mess.wait)
